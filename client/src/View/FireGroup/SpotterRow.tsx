	...
	function SpotterRow( props: { index: number; leg: PolarCoordinate; onRemove: () => void; } )
	{
		...
		return (
			<div className="grid grid-cols-[1fr_1fr_auto] items-center gap-x-2 gap-y-1">
				{/* Labels (row 1) share the SAME grid as inputs */}
				<span className="col-start-1 text-[11px] uppercase tracking-wide text-zinc-400">
					{`S${index + 1}->S${index} Distance`}
				</span>
				<span className="col-start-2 text-[11px] uppercase tracking-wide text-zinc-400">
					Azimuth
				</span>
