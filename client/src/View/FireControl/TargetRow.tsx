...
export const TargetRow = observer(
	function TargetRow( props: { fireGroup: FireGroup; model: Target; index: number; onRemove: () => void; onSelect: () => void } )
	{
		...
		// Render
		let tempClass = "grid gap-2 rounded-md border p-2 bg-zinc-800/70 hover:bg-zinc-800/90 cursor-pointer select-none focus:outline-none focus:ring-2 focus:ring-emerald-500/30";
		const tempIsActive = fireGroup.ActiveTarget === model;

		if ( tempIsActive )
		{
			tempClass += " border-emerald-600/70";
		}
		else
		{
			tempClass += " border-zinc-800";
		}
		...
		return (
			...
			>
				...
				<div className="grid grid-cols-[1fr_auto_auto] items-center gap-2">
					...
					/>
					{tempIsActive && (
						<span className="px-2 py-0.5 text-xs rounded bg-emerald-600/20 text-emerald-300">Active</span>
					)}
					<button
